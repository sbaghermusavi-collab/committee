...
public class UniDate implements Serializable {
	...
	public static DateTimeDTO.DateTimeRs convert(Date date) {
		final UniDate udt = baseConvert(date);

		final DateTimeFieldVO fields = udt.getDateTimeFields();
		final int dow = udt.get(EUniDateField.DAY_OF_WEEK);

		DateTimeDTO.DateTimeRs result = new DateTimeDTO.DateTimeRs()
			.setDayOfWeek(EDayOfWeek.get(dow))
			.setCalendar(ECalendar.of(udt.getCalendar()));

		result
			.setYear(fields.getYear())
			.setMonth(fields.getMonth())
			.setDay(fields.getDay())

			.setHour(fields.getHour())
			.setMinute(fields.getMinute())
			.setSecond(fields.getSecond())
		;

		return result;
	}

	public static DateTimeDTO.DateTimeStrRs convert(Date date, String datePattern, String timePattern) {
		final UniDate udt = baseConvert(date);

		return new DateTimeDTO.DateTimeStrRs()
			.setCalendar(ECalendar.of(udt.getCalendar()))
			.setDate(udt.format(datePattern))
			.setTime(udt.format(timePattern));
	}

	public static String convert(Date date, String pattern) {
		return baseConvert(date).format(pattern);
	}

	public static Date convert(DateTimeDTO.DateTimeRq dateTimeRq) {
		final NICICOBaseContext ctx = NICICOBaseContext.get();

		return UniDate
			.of(
				ctx.getCalendarSafely().getUniCalendar(),
				java.util.TimeZone.getTimeZone(ctx.getTimeZoneIdSafely()))
			.setDate(dateTimeRq.getYear(), dateTimeRq.getMonth(), dateTimeRq.getDay())
			.setTime(dateTimeRq.getHour(), dateTimeRq.getMinute(), dateTimeRq.getSecond())
			.toDate();
	}

	public static Date convert(DateTimeDTO.DateTimeStrRq dateTimeStrRq) {
		final NICICOBaseContext ctx = NICICOBaseContext.get();

		final Integer[] dates = Stream.of(dateTimeStrRq.getDate().split("[/]"))
			.map(Integer::valueOf)
			.toArray(Integer[]::new);

		final Integer[] times = !StringUtils.isEmpty(dateTimeStrRq.getTime()) ?
			Stream.of(dateTimeStrRq.getTime().split("[:]"))
				.map(Integer::valueOf)
				.toArray(Integer[]::new) :
			new Integer[]{0, 0, 0};
	...
	private static UniDate baseConvert(Date date) {
		final NICICOBaseContext currentContext = NICICOBaseContext.get();

		return UniDate
			.of(date)
			.updateCalendar(currentContext.getCalendarSafely().getUniCalendar())
			.updateTimeZone(java.util.TimeZone.getTimeZone(currentContext.getTimeZoneIdSafely()));
